...
class VerificationController extends Controller
{
    ...
    | manager that recently registered with the application. Emails may also
    | be re-sent if the manager didn't receive the original email message.
    |
    */
    ...
    /**
     * Where to redirect managers after verification.
     *
     * @var string
     */
    ...
    {
        if (! hash_equals((string) $request->route('id'), (string) $request->user('manager')->getKey())) {
            throw new AuthorizationException;
        }

        if (! hash_equals((string) $request->route('hash'), sha1($request->user('manager')->getEmailForVerification()))) {
            throw new AuthorizationException;
        }
